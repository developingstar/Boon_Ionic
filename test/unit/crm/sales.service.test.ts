...
describe('SalesService', () => {
  ...
  describe('leads', () => {
    it('returns an API collection', async(() => {
      ...
      service.leads().subscribe((result: PaginatedCollection<Lead>) => {
        ...
        expect(result.items[0]).toEqual(lead)
    ...
    it('allows to use a custom URL', async(() => {
    ...
    it('allows to use a custom params', async(() => {
      const emptyParams = new HttpParams()
  ...
  describe('pipelines', () => {
    it('returns an array', async(() => {
      ...
      service.pipelines().subscribe((result: ReadonlyArray<Pipeline>) => {
        expect(result).toEqual([convertedPipeline, withoutResponsePipeline])
      })
  ...
  describe('createPipeline', () => {
    it('returns a new pipeline', async(() => {
  ...
  describe('createStage', () => {
    it('returns a new stage', async(() => {
      service.createStage(123, { name: 'Closed - Won' }).subscribe((stage) => {
  ...
  describe('lead', () => {
    it('returns a Lead', async(() => {
      service.lead(1).subscribe((lead) => {
        ...
        expect(lead.created_by_user_id).toEqual(101)
        expect(lead.created_by_service_id).toBeNull()
  ...
  describe('updateLead', () => {
    it('returns an updated lead', async(() => {
  ...
  describe('stage', () => {
    it('returns a Stage', async(() => {
      service.stage(1).subscribe((stage) => {
        expect(stage.id).toEqual(1)
        expect(stage.name).toEqual('Closed - Won')
        expect(stage.pipeline_id).toEqual(102)
      })
  ...
  describe('stages', () => {
    describe('when pipeline id is missing', () => {
      it('returns all stages', async(() => {
        service.stages().subscribe((stages) => {
          ...
          expect(stages[0].pipeline_id).toEqual(1)
          expect(stages[1].id).toEqual(12)
          expect(stages[1].name).toEqual('Closed - Won')
          expect(stages[1].pipeline_id).toEqual(2)
        })
    ...
    describe('when pipeline id is given', () => {
      it('returns stages in specified pipeline', async(() => {
        service.stages(103).subscribe((stages) => {
          expect(stages.length).toEqual(2)
          expect(stages[0].id).toEqual(11)
          expect(stages[0].name).toEqual('Signing')
          expect(stages[0].pipeline_id).toEqual(103)
          expect(stages[1].id).toEqual(12)
          expect(stages[1].name).toEqual('Closed - Won')
          expect(stages[1].pipeline_id).toEqual(103)
        })
