...
describe('TextTemplate', () => {
  describe('constructor', () => {
    it('ensures content is a non-empty string', () => {
      expect(() => sampleTextTemplate({ content: null })).toThrow(
        new Error('Expected null to be a non-empty string')
      )
      expect(() => sampleTextTemplate({ content: '' })).toThrow(
        new Error('Expected  to be a non-empty string')
      )
      expect(() => sampleTextTemplate({ content: 1 })).toThrow(
        new Error('Expected 1 to be a non-empty string')
      )
    })

    it('ensures default sender is a phone number', () => {
      expect(() => sampleTextTemplate({ default_sender: null })).toThrow(
        new Error('Expected null to be a phone number')
      )
      expect(() =>
    ...
    it('ensures id is a number', () => {
      expect(() => sampleTextTemplate({ id: null })).toThrow(
        new Error('Expected null to be a number')
      )
      expect(() => sampleTextTemplate({ id: '1' })).toThrow(
        new Error('Expected 1 to be a number')
      )
    })

    it('ensures name is a non-empty string', () => {
      expect(() => sampleTextTemplate({ name: null })).toThrow(
        new Error('Expected null to be a non-empty string')
      )
      expect(() => sampleTextTemplate({ name: '' })).toThrow(
        new Error('Expected  to be a non-empty string')
      )
      expect(() => sampleTextTemplate({ name: 1 })).toThrow(
        new Error('Expected 1 to be a non-empty string')
      )
    })
  })
