...
export class GroupsPage extends ReactivePage<State, UserAction> {
  ...
  protected reduce(state: State, action: UserAction): Observable<State> {
    ...
    if (action.name === 'list') {
      ...
      const group_user = state.groupUsers.find((user) => user.id === Number(action.user.id))
      if (group_user) {
        return Observable.of(state)
      } else {
        return this.groupsService
          ...
          .map<{
            readonly data: {
              ...
              ...state,
              group_users: state.groupUsers.concat(action.user)
            }))
      }
      ...
      return this.groupsService
        ...
        .map<{
          readonly data: {
            ...
            ...state,
            group_users: state.groupUsers.filter((user, ind) => ind !== index)
          }))
    } else {
      return Observable.of(state)
    }
