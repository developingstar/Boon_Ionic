  ...
  protected reduce(state: IState, action: UserAction): Observable<IState> {
    switch (action.name) {
      ...
      case 'delete_template':
        return this.service
          .deleteTemplate(action.template.id)
          .map<
            {
              readonly data: {
          ...
          .catch((error: any, caught: Observable<IState>): ObservableInput<
            IState
          > => {
            if (error.status === 422) {
              ...
              if (errors) {
                ...
                showToast(this.toastController, detail, 2000, false)
              }
            } else {
              showToast(this.toastController, 'Unknown issue', 2000, false)
            }
